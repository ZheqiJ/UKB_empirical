...
def write_stata(results: dict[str, dict[str, object]], raw: list[dict[str, object]]) -> None:
    lines = [
        "Application-level modified Poisson (Poisson QMLE)",
        "Dependent variables: curated 52, broad 55, candidate-inclusive upper-bound attribution outcome.",
        "Timing: exact application start date; PostJuly2024 = 1[start_date >= 2024-07-05].",
        "SE: CR1 sandwich clustered by application start month. Month-of-year fixed effects included.",
        "",
    ]
    for outcome in OUTCOME_META:
        ...
        lines += [OUTCOME_META[outcome]["label"], "".ljust(100, "-")]
        lines.append(f"{'term':<24}{'beta':>13}{'cluster SE':>13}{'z':>10}{'P>|z|':>12}{'[95% CI]':>25}")
        for term in DISPLAY_TERMS:
            month = int(term[-2:]) if term.startswith("month_") else None
            if term not in names:
                note = "omitted: perfect zero prediction" if month in separated_months else "reference month"
                lines.append(f"{term:<24}{note:>76}")
                continue
            index = names.index(term)
            stats = inference_fields(float(beta[index]), float(vcov[index][index]))
            lines.append(f"{term:<24}{stats['estimate']:>13.6f}{stats['se']:>13.6f}{stats['z']:>10.3f}{stats['p_value']:>12.6g}{stats['ci_low']:>12.6f}  {stats['ci_high']:>10.6f}")
        separated_text = ", ".join(str(month) for month in sorted(separated_months)) or "none"
        lines.append(f"Finite-estimation N = {fit['n']} (analytic universe = {fit['analytic_universe_n']}); start-month clusters = {fit['cluster_count']}; month FE = yes; zero-event separated months = {separated_text}; Pearson dispersion = {fit['pearson_dispersion']:.4f}")
        lines.append("")
